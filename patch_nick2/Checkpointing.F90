...
Module Checkpointing
    ...
    Character*6 :: vrchar = 'VR', vtchar = 'VT', tchar = 'T', vpchar = 'VP', rhochar='RHO', achar = 'A', cchar = 'C'
    Character*6 :: vrab = 'VR_AB', vtab = 'VT_AB', tempab = 'T_AB', vpab = 'VP_AB', rhoab='RHO_AB', avarab = 'A_AB', cvarab = 'C_AB'
    Character*3 :: wchar='W', zchar='Z', pchar='P'
    ...
    Subroutine Write_Checkpoint(abterms,iteration,dt,new_dt,elapsed_time)
        ...
        Else
            ...
            Enddo
                ...
                Call Write_Field(rowstrip,1,vrchar, iteration)
                Call Write_Field(rowstrip,2,vtchar, iteration)
                Call Write_Field(rowstrip,3,vpchar, iteration)
                Call Write_Field(rowstrip,4,tchar, iteration)
                Call Write_Field(rowstrip,5,rhochar, iteration)

                offset_index = 4
                If (magnetism) Then
                    Call Write_Field(rowstrip,6,cchar, iteration)
                    Call Write_Field(rowstrip,7,achar, iteration)
                    offset_index = 6
                Endif

                Call Write_Field(rowstrip,offset_index+1,vrab, iteration)
                Call Write_Field(rowstrip,offset_index+2,vtab, iteration)
                Call Write_Field(rowstrip,offset_index+3,vpab, iteration)
                Call Write_Field(rowstrip,offset_index+4,tempab, iteration)
                Call Write_Field(rowstrip,offset_index+5,rhoab, iteration)
                If (magnetism) Then
                    Call Write_Field(rowstrip,offset_index+6,cvarab, iteration)
                    Call Write_Field(rowstrip,offset_index+7,avarab, iteration)
                Endif
            DeAllocate(rowstrip)
        ...
        If (my_row_rank .eq. 0) Then
            !If (nr_read .gt. 0) Then    ! SMALL BUG HERE RELATED TO MPI_IO Logic... -- Revist this
                ...
                If (read_hydro .eq. 1) Then
                    Call Read_Field(rowstrip,1,vrchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,2,vtchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,3,vpchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,4,tchar, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,5,rhochar, iteration,nr_read,nlm_total_old)
                Endif
                offset_index = 4
                If (magnetism) Then
                    If (read_magnetism .eq. 1) Then
                        Call Read_Field(rowstrip,6,cchar, iteration,nr_read,nlm_total_old)
                        Call Read_Field(rowstrip,7,achar, iteration,nr_read,nlm_total_old)
                    Endif
                    offset_index = 6
                Endif

                If (read_hydro .eq. 1) Then
                    Call Read_Field(rowstrip,offset_index+1,vrab, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,offset_index+2,vtab, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,offset_index+3,vpab, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,offset_index+4,tempab, iteration,nr_read,nlm_total_old)
                    Call Read_Field(rowstrip,offset_index+5,rhoab, iteration,nr_read,nlm_total_old)
                Endif
                If (magnetism) Then
                    If (read_magnetism .eq. 1) Then
                        Call Read_Field(rowstrip,offset_index+6,cvarab, iteration,nr_read,nlm_total_old)
                        Call Read_Field(rowstrip,offset_index+7,avarab, iteration,nr_read,nlm_total_old)
                    Endif
                Endif
