...
Contains
    ...
    Subroutine rlm_spacea()
        ...
        ! vr
        DO_IDX2
            SBUFFA(IDX2,hvr) = l_l_plus1(m:l_max)*SBUFFA(IDX2,vr)*Over_RhoRSQ(r)
        END_DO

        ! vtheta
        DO_IDX2
            SBUFFA(IDX2,hvtheta) = l_l_plus1(m:l_max)*SBUFFA(IDX2,vtheta)*Over_RhoRSQ(r)
        END_DO

        ! vphi
        DO_IDX2
            SBUFFA(IDX2,hvphi) = l_l_plus1(m:l_max)*SBUFFA(IDX2,vphi)*Over_RhoRSQ(r)
        END_DO

        ! T
        DO_IDX2
            SBUFFA(IDX2,htvar) = l_l_plus1(m:l_max)*SBUFFA(IDX2,tvar)*Over_RhoRSQ(r)
        END_DO
