...
Contains
    Subroutine physical_space()
        ...
        Allocate(divu(1:n_phi,my_r%min:my_r%max,my_theta%min:my_theta%max))
    ...
    Subroutine Div_dot_V()
        Implicit None
        Integer :: t, r,k

        !Note -- divu is allocated already before calling this routine
        DO_IDX
            divu(IDX) = wsp%p3a(IDX,vr)*two_over_r(r)+wsp%p3a(IDX,dvrdr) + &
                        One_Over_R(r)*(cottheta(t)*wsp%p3a(IDX,vtheta)+wsp%p3a(IDX,dvtdt)) + &
                        One_Over_R(r)*csctheta(t)*wsp%p3a(IDX,dvpdp)
        END_DO 


    End Subroutine Div_dot_V
    ...
    Subroutine Velocity_Advection()
        ...
        ! Theta component:  -[u dot grad u]_phi
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vphi) = -FIELDSP(IDX,vr)*FIELDSP(IDX,dvpdr) &
                - FIELDSP(IDX,vtheta)* ( FIELDSP(IDX,dvpdt) & 
                + One_Over_R(r)*FIELDSP(IDX,vphi)*cottheta(t) ) & 
                - One_Over_R(r)*FIELDSP(IDX,vphi) & 
    ...
    Subroutine Pressure_Force()
        Implicit None
        Integer :: t, r,k
        Real*8 :: afactor, bfactor
        ! Add terms like du/dt = a Grad T + b T Grad lnrho

        ! a and b are declared here so the code compiles.  They depend on gamma, cv, etc.
        ! We will adjust them later...
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr) +afactor*FIELDSP(IDX,dtdr) &
                                        +bfactor*FIELDSP(IDX,tvar)*FIELDSP(IDX,drhodr)
        END_DO
        !$OMP END PARALLEL DO
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta) +One_Over_R(r) & 
                                *( afactor*FIELDSP(IDX,dtdt) &
                                        +bfactor*FIELDSP(IDX,tvar)*FIELDSP(IDX,drhodt) )
        END_DO
        !$OMP END PARALLEL DO
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vphi) = RHSP(IDX,vphi) +One_Over_R(r)*csctheta(t) &
                                *( afactor*FIELDSP(IDX,dtdp) &
                                +bfactor*FIELDSP(IDX,tvar)*FIELDSP(IDX,drhodp) )
        END_DO
        !$OMP END PARALLEL DO
    ...
    Subroutine Velocity_Diffusion()
        ...
        Integer :: dmudr,dmudt,dmudp

        ! muvar = ln(mu) and nuvar = ln(nu)
        Integer :: dmuvardr,dmuvardt,dmuvardp
        Integer :: dnuvardr,dnuvardt,dnuvardp
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr)+( FIELDSP(IDX,d2vrdr2)+Two_Over_R(r)*FIELDSP(IDX,dvrdr) &
                                        +OneOverRSquared(r)*cottheta(t)*FIELDSP(IDX,dvrdt) &
                                        +OneOverRSquared(r)*FIELDSP(IDX,dvrdtdt) &
                                        +OneOverRSquared(r)*csctheta(t)*csctheta(t)*FIELDSP(IDX,dvrdpdp) )*nu(r)
        END_DO
        !$OMP END PARALLEL DO
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr) -2.0d0*OneOverRsquared(r)*nu(r)*( &
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr) + nu(r)*One_Third*(                   &
                           FIELDSP(IDX,d2vrdr2)                               &
                           +Two_Over_R(r)*FIELDSP(IDX,dvrdr)                  &
                           -Two*OneOverRsquared(r)*FIELDSP(IDX,vr)            &
                           -OneOverRsquared(r)*FIELDSP(IDX,dvtdt)             & 
                           +One_Over_R(r)*FIELDSP(IDX,d2vtdrdt)               &
                           -OneOverRsquared(r)*csctheta(t)*FIELDSP(IDX,dvpdp) &
                           +One_Over_R(r)*csctheta(t)*FIELDSP(IDX,d2vpdrdp)   &
                           +One_Over_R(r)*cottheta(t)*FIELDSP(IDX,dvtdr)      &
                           -Two*OneOverRsqured(r)*cottheta(t)*FIELDSP(IDX,vt) )
                        
                           
        END_DO
        !$OMP END PARALLEL DO
        
        !Vector-Tensor Product (grad mu dot e)
        !Radial component 
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr)+Two*(FIELDSP(IDX,dmudr)*FIELDSP(IDX,dvrdr) &
                        (1.0d0/2.0d0)*One_Over_R(r)*(FIELDSP(IDX,dvtdr)-One_Over_R(r) &
                        *FIELDSP(IDX,vtheta)+One_Over_R(r)*FIELDSP(IDX,dvrdt))*FIELDSP(IDX,dmudt) &
                         +(1.0d0/2.0d0)*One_Over_R(r)*csctheta(t)*(One_Over_R(r)*csctheta(t) &
                        *FIELDSP(IDX,dvrdp)+FIELDSP(IDX,dvpdr)-One_Over_R(r)*FIELDSP(IDX,vphi)) &
                        *FIELDSP(IDX,dmudp))
        END_DO
        ...
        ! 1st: nu*Del^2(u_t)
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta)+(FIELDSP(IDX,d2vtdr2)+Two_Over_R(r) &
                        *FIELDSP(IDX,dvtdr)+OneOverRSquared(r) &
                        *(cottheta(t)*FIELDSP(IDX,dvtdt) &
                        +FIELDSP(IDX,dvtdtdt)+csctheta(t) &
                        *csctheta(t)*FIELDSP(IDX,dvtdpdp) ))*nu(r)
        END_DO 

        ! 2nd: Add geometric terms to make this nu*[Del^2{u}]_t
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta) -OneOverRSquared(r)*nu(r)*( &
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        !Changed vr to vtheta inside the RHSP definition. Was this a typo?
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta) + nu(r)*One_Third*(            &
                           One_Over_R(r)*FIELDSP(IDX,d2vrdrdt)          &
                           + Two*OneOverRSquared(r)*FIELDSP(IDX,dvrdt) &
                           + OneOverRSquared(r)*FIELDSP(IDX,d2vtdt2)    &
                           + OneOverRSquared(r)*cottheta(t)*FIELDSP(IDX,dvtdt) &
                           - csctheta(t)*csctheta(t)*OneOverRSquared(r)*FIELDSP(IDX,vtheta) &
                           + OneOverRSquared(r)*csctheta(t)*FIELDSP(IDX,d2vpdtdp) &
                           -csctheta(t)*cottheta(t)*OneOverRSquared(r)*FIELDSP(IDX,dvpdp))
        ...
        !theta component
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta)+Two*((1.0d0/2.0d0)*(FIELDSP(IDX,dvtdr) - &
                          -One_Over_R(r)*FIELDSP(IDX,vtheta)               &
                          +One_Over_R(r)*FIELDSP(IDX,dvrdt))*FIELDSP(IDX,dmudr) &
                          +OneOverRSquared(r)*(FIELDSP(IDX,dvtdt)  &
                          +FIELDSP(IDX,vr))*FIELDSP(IDX,dmudt)     &
                          +(1.0d0/2.0d0)*OneOverRSquared(r)*csctheta(t) &
                          *(FIELDSP(IDX,dvpdt)-cottheta(t)*FIELDSP(IDX,vphi) &
                          +csctheta(t)*FIELDSP(IDX,dvtdp))*FIELDSP(IDX,dmudp) )
        END_DO

        ! Phi DIFFUSION
        ! 1st: nu*Del^2(u_p)
        DO_IDX
            RHSP(IDX,vphi) = RHSP(IDX,vphi)+(FIELDSP(IDX,d2vpdr2)+Two_Over_R(r) &
                        *FIELDSP(IDX,dvpdr)+OneOverRSquared(r) &
                        *(cottheta(t)*FIELDSP(IDX,dvpdt) &
                        +FIELDSP(IDX,dvpdtdt)+csctheta(t) &
                        *csctheta(t)*FIELDSP(IDX,dvpdpdp) ))*nu(r)
        END_DO
        ...
        DO_IDX
            RHSP(IDX,vphi) = RHSP(IDX,vphi)+csctheta(t)*OneOverRSquared(r) &
                        ...
                        *FIELDSP(IDX,dvtdp)  )*nu(r)
        END_DO

        !$OMP END PARALLEL DO

        ! DO SAME FOR THETA AND PHI.  We need some other bits related to compressibiliy
        ! I will do these myself when I return -- they are pretty tricky and we don't have
        ! quite all the relevant code in Diagnostics.F90

        !3rd: Add phi component of gradient of div dot v (1/3 grad div.v)
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vphi) = RHSP(IDX,vphi)+ nu(r)*One_Third*(    &
                         Two*OneOverRSquared(r)*csctheta(t)*FIELDSP(IDX,dvrdp) &
                         +OneOverRSquared(r)*csctheta(t)*FIELDSP(IDX,d2vtdtdp) &
                         +OneOverRSquared(r)*csctheta(t)*cottheta(t)*FIELDSP(IDX,dvtdp) &
                         +OneOverRsquared(r)*csctheta(t)*csctheta(t)*FIELDSP(IDX,d2vpdp2) &
        ...
        !phi component
        DO_IDX
            RHSP(IDX,vphi) = RHSP(IDX,vphi)+Two*((1.0d0/2.0d0)   &
                         *(One_Over_R(r)*csctheta(t)*FIELDSP(IDX,dvrdp) &
                         + FIELDSP(IDX,dvpdr)           &
                         - One_Over_R(r)*FIELDSP(IDX,vphi))*FIELDSP(IDX,dmudr) &
                         + (1.0d0/2.0d0)*OneOverRSquared(r)*(FIELDSP(IDX,dvpdt) &
                         - cottheta(t)*FIELDSP(IDX,vphi) & 
                         + csctheta(t)*FIELDSP(IDX,dvtdp))*FIELDSP(IDX,dmudt)
                         + OneOverRSquared(r)*csctheta(t)     &
                         * (csctheta(t)*FIELDSP(IDX,dvpdp)    &
                         + FIELDSP(IDX,vr)                   &
                         + cottheta(t)*FIELDSP(IDX,vtheta))*FIELDSP(IDX,dmudp) )
        END_DO

        !muvar components
        !Double check with Nick if this is correct and if the placement is
        !right. 
        !radial component
        DO_IDX
            RHSP(IDX,dmuvardr) = RHSP(IDX,dmuvardr) + FIELDSP(IDX,drhodr) &
                                + FIELDSP(IDX,dnuvardr)
        END_DO
        
        !theta component
        DO_IDX
            RHSP(IDX,dmuvardt) = RHSP(IDX,dmuvardt) + FIELDSP(IDX,drhodt) &
                                + FIELDSP(IDX,dnuvardt)
        END_DO

        !Phi component
        DO_IDX
            RHSP(IDX,dmuvardp) = RHSP(IDX,dmuvardp)
        END_DO
    ...
    Subroutine Coriolis_Centrifugal()
        ...
        ! The Coriolis coeff will be 1/Ekman or 2 Omega
        ! Not sure if I should multiply by density similar to the centrifugal
        ! branch since equations don't have density in them. Leaving it without
        ! density for now. 
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta) -2.0d0* &
                & ref%Coriolis_Coeff*costheta(t)*FIELDSP(IDX,vphi)
        END_DO
        !$OMP END PARALLEL DO

        ! Coriolis:  Phi  (You figure out the formula for this one :)
        ...
        DO_IDX
            RHSP(IDX,vr) = RHSP(IDX,vr) - &
                ref%Centrifugal_Coeff*sintheta(t)*radius(r)
        END_DO
        !$OMP END PARALLEL DO

        ! Centrifugal:  Theta  (hint see how this is implemented in
        !      the centrifugal branch of the code you've been working on...)
        !      (DOUBLE HINT:  double check signs -- we are no longer working with
        !                     stream functions... -- those equations can change signs, 
        !                     add density factors etc.)
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,vtheta) = RHSP(IDX,vtheta) + &
                ref%Centrifugal_Coeff*costheta(t)*radius(r)
        END_DO
        !$OMP END PARALLEL DO
    ...
    Subroutine Density_Compression()
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,rhovar) = RHSP(IDX,rhovar)-divu(IDX)
        END_DO
        !$OMP END PARALLEL DO
    ...
    Subroutine Temperature_Viscous_Heating()
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,tvar) = RHSP(IDX,tvar) ! + PHI term
        END_DO
        !$OMP END PARALLEL DO
    ...
    Subroutine Temperature_Compression()
        ...
        !$OMP PARALLEL DO PRIVATE(t,r,k)
        DO_IDX
            RHSP(IDX,tvar) = RHSP(IDX,tvar) + divu(IDX)! + div V term
        END_DO
        !$OMP END PARALLEL DO
