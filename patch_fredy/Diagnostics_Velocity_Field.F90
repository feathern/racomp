...
Contains
    ...
    Subroutine Compute_Velocity_Components(buffer)
        ...
        If (compute_quantity(rhov_r)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,vr)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovp_r)) Then
            DO_PSI
                qty(PSI) = fbuffer(PSI,vr)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovm_r)) Then
            DO_PSI
                qty(PSI) = m0_values(PSI2,vr)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhov_theta)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,vtheta)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovp_theta)) Then
            DO_PSI
                qty(PSI) = fbuffer(PSI,vtheta)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovm_theta)) Then
            DO_PSI
                qty(PSI) = m0_values(PSI2,vtheta)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhov_phi)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,vphi)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovp_phi)) Then
            DO_PSI
                qty(PSI) = fbuffer(PSI,vphi)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rhovm_phi)) Then
            DO_PSI
                qty(PSI) = m0_values(PSI2,vphi)*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
