    ...
    Subroutine Compute_TurbulentKE_Budget(buffer)
        ...
        If (compute_quantity(production_shear2_pKE)) Then
            DO_PSI2
                ...
                DO k = 1, n_phi
                    ...
                    qty(PSI) = -buffer(PSI,rhovar) * qty(PSI)
        ...
        If (compute_quantity(rflux_turbadvect_pKE)) Then
            DO_PSI
                ...
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * fbuffer(PSI,vr)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(thetaflux_turbadvect_pKE)) Then
            DO_PSI
                htmp1 = fbuffer(PSI,vr)**2 + fbuffer(PSI,vtheta)**2 + fbuffer(PSI,vphi)**2
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * fbuffer(PSI,vtheta)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(phiflux_turbadvect_pKE)) Then
            DO_PSI
                htmp1 = fbuffer(PSI,vr)**2 + fbuffer(PSI,vtheta)**2 + fbuffer(PSI,vphi)**2
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * fbuffer(PSI,vphi)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(rflux_meanadvect_pKE)) Then
            DO_PSI
                htmp1 = fbuffer(PSI,vr)**2 + fbuffer(PSI,vtheta)**2 + fbuffer(PSI,vphi)**2
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * m0_values(PSI2,vr)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(thetaflux_meanadvect_pKE)) Then
            DO_PSI
                htmp1 = fbuffer(PSI,vr)**2 + fbuffer(PSI,vtheta)**2 + fbuffer(PSI,vphi)**2
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * m0_values(PSI2,vtheta)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(phiflux_meanadvect_pKE)) Then
            DO_PSI
                htmp1 = fbuffer(PSI,vr)**2 + fbuffer(PSI,vtheta)**2 + fbuffer(PSI,vphi)**2
                qty(PSI) = 0.5D0*buffer(PSI,rhovar) * htmp1 * m0_values(PSI2,vphi)
            END_DO
            Call Add_Quantity(qty)
        Endif
