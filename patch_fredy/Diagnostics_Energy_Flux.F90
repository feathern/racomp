...
Contains
    ...
    Subroutine Compute_Energy_Flux(buffer)
        ...
        If (compute_quantity(visc_flux_r)) Then
            ...
            DO_PSI
                qty(PSI) = qty(PSI)*nu(r)*buffer(PSI,rhovar)
            END_DO
        ...
        If (compute_quantity(ke_flux_radial)) Then
            ...
            DO_PSI
                qty(PSI) = qty(PSI)*buffer(PSI,rhovar)*0.5d0
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(Enth_flux_radial)) Then
            ...
            Do t = my_theta%min, my_theta%max
                Do r = my_r%min, my_r%max
                    ...
                    dt_by_dp = 1.0d0/pressure_specific_heat/buffer(PSI,rhovar)
                    Do k = 1, n_phi
                        tpert = dt_by_ds*buffer(PSI,tvar) &
                         & + dt_by_dp*buffer(PSI,pvar)
                        tpert = tpert*buffer(PSI,rhovar) ! This is now T'*rho_bar
        ...
        If (compute_quantity(thermalE_flux_radial)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,vr)*buffer(PSI,tvar)* &
                           & buffer(PSI,rhovar)*ref%temperature(r)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(vol_heat_flux)) Then
            ...
            If (heating_type .gt. 0) Then
                ...
                Do r = N_R-1, 1,-1
                    qadd = ref%heating(r)*buffer(PSI,rhovar)*ref%temperature(r) ! the heat
