...
Contains
    ...
    Subroutine Compute_Coriolis_Force(buffer)
        ...
        If(compute_quantity(Coriolis_pForce_r)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*coriolis_term*sintheta(t)*fbuffer(PSI,vphi)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If(compute_quantity(Coriolis_mForce_r)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*coriolis_term*sintheta(t)*m0_values(PSI2,vphi) &
        ...
        If (compute_quantity(Coriolis_pForce_Theta)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*coriolis_term*costheta(t)*fbuffer(PSI,vphi)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(Coriolis_mForce_Theta)) Then
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*coriolis_term*costheta(t)*m0_values(PSI2,vphi) ! &
        ...
        If (compute_quantity(Coriolis_pForce_Phi)) Then
            DO_PSI
                qty(PSI) = - (coriolis_term*costheta(t)*fbuffer(PSI,vtheta) &
                           + coriolis_term*sintheta(t)*fbuffer(PSI,vr))*buffer(PSI,rhovar)
            END_DO
            Call Add_Quantity(qty)
        Endif
        ...
        If (compute_quantity(Coriolis_mForce_Phi) .or. compute_quantity(samom_coriolis)) Then
            DO_PSI
                qty(PSI) = - (coriolis_term*costheta(t)*m0_values(PSI2,vtheta) &
                           + coriolis_term*sintheta(t)*m0_values(PSI2,vr))*buffer(PSI,rhovar)
            END_DO
