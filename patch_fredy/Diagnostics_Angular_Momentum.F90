...
Contains
    ...
    Subroutine Compute_Angular_Momentum(buffer)
        ...
        If (compute_quantity(amom_z)) Then
            ...
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*radius(r)*sintheta(t) &
                    & *buffer(PSI,vphi)
            END_DO
        ...
        If (compute_quantity(amom_x)) Then
            ...
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*radius(r)*&
    ...
    Subroutine Compute_Angular_Momentum_Fluxes(buffer)
        ...
        If (compute_quantity(famom_mean_r)) Then
            ...
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*((radius(r)*sintheta(t))**2) &
        ...
        If (compute_quantity(famom_diff_r)) Then
            ...
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*sintheta(t)*nu(r)*(m0_values(PSI2,vphi)-radius(r)*m0_values(PSI2,dvpdr))
            END_DO
        ...
        If (compute_quantity(famom_diff_theta)) Then
            ...
            DO_PSI
                qty(PSI) = buffer(PSI,rhovar)*nu(r)*(costheta(t)*m0_values(PSI2,vphi)-sintheta(t)*m0_values(PSI2,dvpdt))
            END_DO
